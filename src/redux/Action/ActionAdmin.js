...
//?get profile admin
export const GetProfileAdminStart = () => ({
  type: types.GET_PROFILE_ADMIN_START,
});
export const GetProfileAdminSuccess = (token) => ({
  type: types.GET_PROFILE_ADMIN_SUCCESS,
  payload: token,
});
export const GetProfileAdminFail = (error) => ({
  type: types.GET_PROFILE_ADMIN_FAIL,
  payload: error,
});
//?clear Error
...
export const loginInitiate = (email, password) => {
  return async function (dispatch) {
    dispatch(LoginStart());
    await axios
      .post(`/api/admin/user/login`, { email, password })
      .then((user) => {
...
//!Refresh_token
export const RefreshTokenInitiate = ({ tokenAdmin }) => {
  return async function (dispatch) {
    dispatch(RefreshTokenStart());
    await axios
      .get(`/api/admin/user/refreshToken`, {
...
//!InfoAdmin
export const InfoAdminInitiate = ({ token, userId }) => {
  return async function (dispatch) {
    dispatch(InfoAdminStart());
    await axios
      .get(`/api/admin/user/${userId}/info`, {
...
export const GetAccountNewInitiate = ({ token }) => {
  ...
  return async function (dispatch) {
    ...
    await axios
      .get(`/api/admin/statistics/registeredUser/recently`, {
        headers: { Authorization: `${token}` },
      })
      .then((user) => {
...
//!Get profile admin
export const GetProfileAdminInitiate = ({ token }) => {
  console.log(token);
  return async function (dispatch) {
    dispatch(GetProfileAdminStart());
    await axios
      .get(`/api/admin/user/profile`, {
        headers: { Authorization: `${token}` },
      })
      .then((user) => {
        dispatch(GetProfileAdminSuccess(user.data.data));
      })
      .catch((error) => {
        dispatch(GetProfileAdminFail(error.data));
      });
  };
};
//! ClearErrorSuccess
